...
void FIFO_method(cache* c, int tag) {
	...
	for (int i = 0; i < Cache_size; i++) {
		if (c->line[i].access_time == 0) {
			c->line[i].tag = tag;
			c->line[i].valid = 1;
			//c->line[i].count = 1;	// 캐시에 데이터를 적재할 때 카운트는 초기화를 하지 않는다.
		...
		if (temp == tag)   //캐시의 태그와 메모리 블록 번호가 일치하는 블록
		{
			for (j = 0; j < Cache_size; j++) {
				if (c->line[j].valid == 0) { //캐시의 유효비트 확인 후 0이면 사용가능
					...
					c->line[j].count++;   //LRFU를 위한 접근 횟수 갱신
					c->line[j].access_time = j; //LRFU 및 FIFO 방식을 위한 접근 시간 갱신
					valid = 1;
					c->count++;   //접근 시간 갱신 변수
					break;
				}
			...
			if (valid == 0) {
				if (menu == 1)
					...
					LRU_method(c, tag);   // LRFU 방식 블록 교체   
				else
					printf("잘못 선택하셨습니다.");
			}
